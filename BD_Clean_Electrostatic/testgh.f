c***************************************************************
      subroutine testgh(lenVec,n,xc,fc,gc,y,yhy,vec,
     ...
     +    Ec_HL2, h_g tc, Eb_tc, withlink, evd_link, evd_hh,
...
c----------------------------------------------------------------------
      ...
      integer n,lenVec
      double precision fc, yhy, xc(lenVec),gc(lenVec)
      double precision y(lenVec),vec(lenVec)
...
c     The 2 subcomponents of Ec_CT: Ec_CT1: tails with its own core; Ec_CT2: tails with other cores 
     ...
     +     Ev_CT1,Ev_CT2,Ev_HH,Ev_HT,Ev_HL1,Ev_HL2,Ev_HC
      ...
      integer i,i1,i2,i3
      double precision rs(n3),as(n3),bs(n3),cs(n3) ! Change coordinates inside function without affect the original coordinates in calling function
      double precision h_Xs(h_n), h_Ys(h_n), h_Zs(h_n)
      double precision t_Xs(t_n), t_Ys(t_n), t_Zs(t_n)
	  double precision core_poss(Nq3)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      double precision matA(3,3), matAinv(3,3),vecC(3,1)!needed for test core pesduo charges to do a perturbation
      double precision new_core_pos(3,1)                !in charges location
      ...
      if (analytic.eq.1) then
          ...
          write (filename, "(A40)") 'TestRes_Bending_Analytic.txt'
           ...
           write (filename, "(A40)") 'TestRes_Twisting_Analytic.txt'
			      ...
			      if(tested_comp.eq.4)  then 
			write (filename, "(A40)") 'TestRes_Elec_LL_Analytic.txt'	  
                  endif
				  if(tested_comp.eq.5)  then 
			write (filename, "(A40)") 'TestRes_Elec_LC_Analytic.txt'	  
                  endif
			      if(tested_comp.eq.6)  then 
			write (filename, "(A40)") 'TestRes_Elec_CC_Analytic.txt'	  
                  endif
                  if(tested_comp.eq.7)  then 
			write (filename, "(A40)") 'TestRes_Elec_HH_Analytic.txt'	  
                  endif
                  if(tested_comp.eq.8)  then 
			write (filename, "(A40)") 'TestRes_Elec_HL1_Analytic.txt'	  
                  endif
				  if(tested_comp.eq.9)  then 
			write (filename, "(A40)") 'TestRes_Elec_HL2_Analytic.txt'	  
                  endif
				  if(tested_comp.eq.10)  then 
			write (filename, "(A40)") 'TestRes_Elec_HC_Analytic.txt'	  
                  endif
				   if(tested_comp.eq.11)  then 
			write (filename, "(A40)") 'TestRes_Elec_TH_Analytic.txt'	  
                  endif
				   if(tested_comp.eq.12)  then 
			write (filename, "(A40)") 'TestRes_Elec_TT_Analytic.txt'	  
                  endif
				   if(tested_comp.eq.13)  then 
			write (filename, "(A40)") 'TestRes_Elec_TL_Analytic.txt'	  
                  endif
				  endif 
              endif
          endif
          ...
          if (tested_comp.eq.1) then
            write (filename, "(A40)") 'TestRes_Stretching_Numerical.txt'
              ...
              write (filename, "(A40)") 'TestRes_Twisting_Numerical.txt'
			       ...
			       if (tested_comp.eq.4) then
              write (filename, "(A40)") 'TestRes_Elec_LL_Numerical.txt'                            
			      endif
				  endif
              endif
          endif       
...
cccc     our objective function is inserted here
      ...
      if(tested_comp.le.6)then ! vec represents the perturbution in DNA beads =r  IF Tested_comp=1,2,3,4,5,6
     ...
     +    np,myid,ierr,100)
      ...
      if (tested_comp.eq.7)   then  
      call update_mod(n_c,nc3,num,n3,type,rs,ro,d1,go,as,bs,cs,
     +        alphas,betas,gammas,lengths,a_dnas,b_dnas,c_dnas,
     +        alpha_ps,beta_ps,gamma_ps,100)
      ...
      do 138 i = 1,h_n
          ...
          h_Zs(i)=vec(i3)
138    continue
      call potential(n_c,num,n3,type,rs,as,bs,cs,alphas,betas,gammas,
     +	  lengths,beta_ps,lo,hd2,gd2,sd2,k_e,k_ex,Nq,Nq3,core_pos,core_q,
     +    debye,q_l,phi_o,Es,t_n,t_chg,t_X, t_Y, t_Z,t_X0, t_Y0, t_Z0,
     +    h_Xs,h_Ys,h_Zs,h_chg,h_n,evd_hcG,evd_hcC,evd_hlG,evd_hlC,
     +    t_bond, t_bond_v, t_bond_c, t_angle, t_angle_v, t_angle_c, 
     +    t_exv_e, t_exv_d, t_Eb, t_Ea, t_Ec, t_Ev,t_grp, t_fix,
     +    vdw_cut, Rcut, evd_tc, evd_tl, evd_cc, evd_cl, evd_ll,
     +    Ec_LL, Ec_CC, Ec_TT, Ec_LC, Ec_CT, Ec_TL,Ec_TT1, 
     ...
     +    np,myid,ierr,100)
      ...
      if( (tested_comp.eq.8)  .or.
     +(tested_comp.eq.9)) then
...
c       vecC(1,1)=vec(i1)-rs(i11)
c       vecC(2,1)=vec(i2)-rs(i22)
c       vecC(3,1)=vec(i3)-rs(i33)


c      call       inverse(matA,matAinv,3)
    
c      new_core_pos=matmul(matAinv,vecC)

c      if(new_core_pos(1,1).eq.0.0)  then ! not affected by force
c          core_poss(i11)=core_pos(i11)
c          core_poss(i22)=core_pos(i11)
c          core_poss(i33)=core_pos(i11)
c       else
c          core_poss(i11)=new_core_pos(1,1)
c          core_poss(i22)=new_core_pos(2,1)
c          core_poss(i33)=new_core_pos(3,1)
c       endif
        core_poss(i11)=vec(i1)
        core_poss(i22)=vec(i2)
        core_poss(i33)=vec(i3)
...
142     continue
     ...
     +        alpha_ps,beta_ps,gamma_ps,100)
      ...
      call potential(n_c,num,n3,type,rs,as,bs,cs,alphas,betas,gammas,
     +lengths,beta_ps,lo,hd2,gd2,sd2,k_e,k_ex,Nq,Nq3,core_poss,core_q,
     +    debye,q_l,phi_o,Es,t_n,t_chg,t_X, t_Y, t_Z,t_X0, t_Y0, t_Z0,
     +    h_Xs,h_Ys,h_Zs,h_chg,h_n,evd_hcG,evd_hcC,evd_hlG,evd_hlC,
     +    t_bond, t_bond_v, t_bond_c, t_angle, t_angle_v, t_angle_c, 
     +    t_exv_e, t_exv_d, t_Eb, t_Ea, t_Ec, t_Ev,t_grp, t_fix,
...
144     continue
     ...
     +        alpha_ps,beta_ps,gamma_ps,100)
      ...
      call potential(n_c,num,n3,type,rs,as,bs,cs,alphas,betas,gammas,
     +lengths,beta_ps,lo,hd2,gd2,sd2,k_e,k_ex,Nq,Nq3,core_poss,core_q,
     + debye,q_l,phi_o,Es,t_n,t_chg,t_Xs, t_Ys, t_Zs,t_X0, t_Y0, t_Z0,
     +    h_Xs,h_Ys,h_Zs,h_chg,h_n,evd_hcG,evd_hcC,evd_hlG,evd_hlC,
     +    t_bond, t_bond_v, t_bond_c, t_angle, t_angle_v, t_angle_c, 
     +    t_exv_e, t_exv_d, t_Eb, t_Ea, t_Ec, t_Ev,t_grp, t_fix,
     +    vdw_cut, Rcut, evd_tc, evd_tl, evd_cc, evd_cl, evd_ll,
